...
python example/manage.py test --settings=test_settings --with-xunit --with-xcover --cover-package=attachments
TEST_EXIT=$?
rm -rf jenkins_reports
mkdir jenkins_reports
pep8 attachments > jenkins_reports/pep8.report
PEP8_EXIT=$?
pyflakes attachments > jenkins_reports/pyflakes.report
PYFLAKES_EXIT=$?
